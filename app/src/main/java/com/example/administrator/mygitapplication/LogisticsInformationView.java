...
//设置渐变色
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(left * 2, top, width + left * 2, heightTotal + top, paint);//(left*2, top)起点，width是矩形的宽度，heightTotal是矩形的高度，首先确认画布的高度绘制矩形（就是贯穿圆心的连接直线,画笔默认是fill模式）
        //画圆
...
//                mPaint.setShader(mLinearGradient);
                ...
                //01-20  (text:要绘制的文字 , x：绘制原点x坐标, y：绘制原点y坐标,paint:用来做画的画笔)heightList.get(i) * 2 / 3 + left
                canvas.drawText(((LogisticsData) data.get(i)).getDate() + "", 5, top + 32, datePaint);
                //11:23 heightList.get(i) * 2 / 3
...
//                LogUtils.i("获取最后一行占的宽度"+layout.getLineWidth(layout.getineCount()-1));
...
//                canvas.drawCircle(width / 2 + left, heightData + 44, radius, paint);
                ...
                texttimePaint.setAntiAlias(true);
                texttimePaint.measureText(((LogisticsData) data.get(i)).getTime());
                //interval + heightList.get(i) * 2 / 3 + left + heightData + top,
                canvas.drawText(((LogisticsData) data.get(i)).getDate() + "", 5,  20 + heightData + top, datePaint);
                //interval + heightList.get(i) * 2 / 3 + heightData + top,
                ...
                if (splitData != null) {
                    ...
                    canvas.save();//很重要，不然会样式出错  保存画布没有移动位置的状态
                    canvas.translate(left * 2 + radius * 2 + 10, heightData + top); //向右移动画布
                    layout.draw(canvas);//画上文字
                    canvas.restore();//重置画布，恢复画布没有移动之前的状态
                }
